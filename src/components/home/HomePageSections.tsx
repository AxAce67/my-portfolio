...
function CountUpNumber({
  ...
  useEffect(() => {
    ...
    if (prefersReducedMotion) {
      if (el) el.textContent = String(end);
      setIsDone(true);
      return;
    }
    ...
    const tick = (time: number) => {
      ...
      if (el) el.textContent = String(Math.round(end * eased));
...
function AboutStatCard({
  ...
  const tiltClassName = `bento-card about-card-motion flex flex-col items-center justify-center ${isInteractive ? 'group cursor-pointer' : ''} ${className}`.trim();
    ...
    return (
      <Link 
        ...
        onClick={() => {
          ...
          writeSessionValue(navigationStateKeys.homeReferrerHash, 'about');
        }}
      >
        <TiltCard className={tiltClassName}>{inner}</TiltCard>
      </Link>
    );
  }
  ...
  if (onActivate) {
    return (
      <button type="button" onClick={onActivate} className="contents text-left">
        <TiltCard className={tiltClassName}>{inner}</TiltCard>
      </button>
    );
  }

  return <TiltCard className={tiltClassName}>{inner}</TiltCard>;
}
...
function MomentumLineChart({
  ...
  // Commit counts are always whole numbers — round the axis max up to the
  // nearest multiple of 4 so both the max and the midpoint tick stay
  // integers (no "7.5" labels).
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="hidden md:grid grid-cols-1 md:grid-cols-4 gap-4 auto-rows-[minmax(120px,auto)] md:auto-rows-[minmax(140px,auto)]">
          ...
          <ScrollReveal delay={0.2} direction="scale">
            <AboutStatCard value={stats[0].value} suffix={stats[0].suffix} label={stats[0].label} href={stats[0].href} onActivate={stats[0].onActivate} durationMs={1200} className="h-full py-5" />
          </ScrollReveal>

          <ScrollReveal delay={0.24} direction="scale">
            <AboutStatCard value={stats[1].value} suffix={stats[1].suffix} label={stats[1].label} href={stats[1].href} onActivate={stats[1].onActivate} durationMs={1300} className="h-full py-5" />
          </ScrollReveal>

          <ScrollReveal delay={0.28} direction="scale">
            <AboutStatCard value={stats[2].value} suffix={stats[2].suffix} label={stats[2].label} href={stats[2].href} onActivate={stats[2].onActivate} durationMs={1400} className="h-full py-5" />
          </ScrollReveal>

          <ScrollReveal delay={0.32} direction="scale">
            <AboutStatCard value={stats[3].value} label={stats[3].label} href={stats[3].href} onActivate={stats[3].onActivate} durationMs={1500} className="h-full py-5" />
          </ScrollReveal>
              ...
              >
                <div className={effectiveViewMode === 'grid' ? 'flex flex-col h-full' : 'flex flex-row h-28 sm:h-44 overflow-hidden'}>
                  ...
                  <div className={`${effectiveViewMode === 'grid' ? 'p-3 sm:p-5' : 'p-3 sm:p-4 flex flex-col justify-center'} flex-1 min-w-0 overflow-hidden`}>
                    ...
                    {!isMobileViewport && project.created_at && (
        ...
        {projects.length > 0 && (
          <div className="mt-6 flex justify-end">
            <TransitionLink 
              ...
              onClick={() => {
                writeSessionValue(navigationStateKeys.homeScrollY, String(window.scrollY));
                writeSessionValue(navigationStateKeys.homeReferrerHash, 'timeline');
              }}
            >
              {t('viewAll')}
            </TransitionLink>
...
function ActiveProjectsSection({
  ...
  const has = (key: string) => {
    const val = t(key);
    return val && val !== key && val !== `ActiveProjects.${key}`;
  };
  const text = (key: string, fallback: string) => (has(key) ? t(key) : fallback);
  const ganttRef = useRef<HTMLDivElement>(null);
  const isGanttInView = useInView(ganttRef, { once: true, margin: '-100px' });
  ...
  return (
    <section id="timeline" className="py-12 sm:py-16 lg:py-20">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div ref={ganttRef} className="hidden md:block gantt-chart">
